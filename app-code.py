...
if st.button("Bereken winkansen"):  
    ...
    header_table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor("#DCCAB7")),  
    ...
    for c in criteria:  
        wt=int(round(criterion_weights.get(c,0.0)))  
        mp=int(round(criterion_maxpoints.get(c,0.0)))  
        jde_score=int(verwachte_scores_eigen.get(c,0))  
        jde_raw=int(round(jde_breakdown[c]["raw_points"]))  
        crit_table_data.append([c, f"{wt}", f"{mp}", f"{jde_score}", f"{jde_raw}"])  

    # Voeg prijs-rij toe  
    prijs_wt = int(round(criterion_weights.get("Prijs", 0)))  
    prijs_mp = int(round(criterion_maxpoints.get("Prijs", 0)))  
    prijs_score = int(round(jde_p))  
    crit_table_data.append(["Prijs", f"{prijs_wt}", f"{prijs_mp}", f"{prijs_score}", f"{prijs_score}"])  

    crit_tbl=Table(crit_table_data, colWidths=[140,70,70,70,80])  
    crit_tbl.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.HexColor("#DCCAB7")),  
                                  ('TEXTCOLOR',(0,0),(-1,0),colors.HexColor("#333")),  
    ...
    t.setStyle(TableStyle([  
        ('BACKGROUND',(0,0),(-1,0),colors.HexColor("#DCCAB7")),  
        ('TEXTCOLOR',(0,0),(-1,0),colors.HexColor("#333")),  
        ('GRID',(0,0),(-1,-1),0.25,colors.HexColor("#FFFAF6")),  
        ('FONTNAME',(0,0),(-1,0),'OswaldBold'),  
        ('FONTSIZE',(0,0),(-1,-1),9),  
