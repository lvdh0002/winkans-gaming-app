...
if st.button("Bereken winkansen"):
    ...
    st.download_button(
        "Download volledige resultaten (CSV)",
        data=csv_bytes,
        file_name="winkans_volledig.csv",
        mime="text/csv"
    )
    ...
    styles.add(ParagraphStyle(name="JDENormal", fontName="Helvetica", fontSize=10, leading=12, textColor=colors.black))
    styles.add(ParagraphStyle(name="JDEItalic", fontName="Helvetica-Oblique", fontSize=9, leading=11, textColor=colors.HexColor("#444")))
    styles.add(ParagraphStyle(name="JDETableHeader", fontName="Helvetica-Bold", fontSize=9, textColor=colors.white, alignment=1))
    flow = []
    ...
    table_data = [[Paragraph(col, styles["JDETableHeader"]) for col in pdf_cols]]
    for r in rows:
        table_data.append([Paragraph(str(r[col]), styles["JDENormal"]) for col in pdf_cols])
    col_widths = [150,60,50,140,140]
    ...
    pdf_buf.seek(0)
    st.download_button(
        "📄 Download compacte JDE one-pager (PDF)",
        data=pdf_buf.getvalue(),
        file_name="winkans_onepager.pdf",
